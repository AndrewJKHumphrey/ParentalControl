...
> **Important — Debug vs Release:**
> The Debug build skips the login window (`#if !DEBUG` in `App.xaml.cs`).
> All testing must use a **Release** binary. There are two ways to get one:
> - **Full install (recommended):** run `install.ps1` as Administrator — this publishes Release and installs the services.
> - **Quick UI test only:** run `run-ui-release.bat` from the repo root — builds and launches the UI in Release without reinstalling services.
---
...
- [ ] Open the **installed** UI (`C:\Program Files\ParentalControl\ParentalControl.UI.exe`) or run `run-ui-release.bat` — login prompt must appear (if it does not, you are running the Debug build)
- [ ] Login with default password `parent1234`
- [ ] Confirm first-run flow completes without error

**Browser Extension (manual load required):**
- [ ] Open Edge → `edge://extensions` → enable **Developer mode** → **Load unpacked** → select the `ParentalControl.Extension` folder
- [ ] Extension shows as enabled with no errors or warnings
- [ ] Open Chrome (if installed) → `chrome://extensions` → enable **Developer mode** → **Load unpacked** → select the same folder
...
**Theme checks — fresh install defaults (verify before changing anything):**
- [ ] Debug build: app opens with **Debug** theme — cool blue-gray sidebar/cards (`#252830`), light blue accents/borders (`#5BB8F5`), cool off-white text (`#E4EEF8`)
- [ ] Release standalone: app opens with **Default** theme — dark blue-purple bg (`#1E1E2E`), muted blue borders (`#585B70`), light lavender text (`#CDD6F4`)
- [ ] Debug theme does not appear in the theme dropdown when running standalone Release
- [ ] Custom theme item only appears in the dropdown in Debug builds

**Named theme spot checks:**
- [ ] Raven: near-black bg (`#0F0F0F`), electric blue accent (`#60AFFF`), white text
- [ ] Deep Ocean: dark teal bg, teal accent
- [ ] Selecting any theme persists across UI restart (saved to DB)
...
- [ ] Manually load extension in Edge: `edge://extensions` → Developer mode on → Load unpacked → select `ParentalControl.Extension` folder
